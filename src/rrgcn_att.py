...
from src.decoder1 import ConvTransE, ConvTransR
...
class RecurrentRGCN(nn.Module):
    ...
    def forward(self,sub_graph,T_idx, query_mask, g_list, static_graph, use_cuda):
        ...
        #-----------------长期历史建模-------------------------------------
        if self.pre_type == "long" or self.pre_type=="all":
        ...
        his_temp_embs =[]
        his_rel_embs =[]
        if self.pre_type=="short" or self.pre_type=="all":
    ...
    def predict(self,que_pair, sub_graph,T_id, test_graph, num_rels, static_graph, test_triplets, use_cuda):
        with torch.no_grad():
            ...
            t1 = torch.tensor(T_id).cuda().to(self.gpu)
            q_t = torch.cos(self.weight_t2 * 0 + self.bias_t2).repeat(self.num_ents,1)
            # qe_emb = self.w4(torch.concat([self.dynamic_emb,q_t],dim=1))
            
            # e1_emb = qe_emb[all_triples[:, 0]]
            # index = torch.unique(torch.concat([uniq_e,subg_index])) ##历史重要实体和查询实体集合
            ...
            #-----------------长期历史建模-------------------------------------
            if self.pre_type == 'long':
                scores_ob1,_ = self.decoder_ob.forward(embedding,his_r_emb, all_triples, his_emb, self.pre_weight, self.pre_type)
                score_h = F.softmax(scores_ob1, dim=1)
                score_en =  score_h 
            #-----------------短期历史建模-------------------------------------
            if self.pre_type == 'short':
            ...
            if self.pre_type == "all":
                ...
                scores_ob,_= self.decoder_ob.forward( embedding,r_emb, all_triples,  his_emb, self.pre_weight, self.pre_type)
                score_seq = F.softmax(scores_ob, dim=1)
                # scores_ob1,_ = self.decoder_ob1.forward(his_emb,his_r_emb, all_triples)
                # score_h = F.softmax(scores_ob1, dim=1)
                # score_en = self.pre_weight*score_seq +(1-self.pre_weight)*score_h
                score_en =score_seq
            scores_en = torch.log(score_en)
            # score_rel = self.rdecoder.forward(self.dynamic_emb,self.emb_rel, all_triples, mode="test")
    ...
    def get_loss(self,que_pair, sub_graph,T_idx, glist, triples, static_graph, use_cuda):
        ...
        # embedding = F.normalize(evolve_embs) if self.layer_norm else evolve_embs
        # r_emb = F.normalize(r_emb) if self.layer_norm else r_emb
        ...
        if self.use_cl and self.pre_type=="all":
            for id, evolve_emb in enumerate(his_temp_embs):
                ...
                # t3 = torch.tensor(t3).cuda().to(self.gpu)
                # cl_t = np.exp(-t3)
        ...
        # if self.use_static:
        #     if self.discount == 1:
        #         for time_step, evolve_emb in enumerate(his_temp_embs):
    ...
    # def all_GCN(self, ent_emb, subg, use_cuda):
    #     ## input：历史子图subg
    #     ## out：子图的表示 F.normalize(subg_emb)
    #     que_subg = []
    #     sub_graph = subg.to(self.gpu)
    #     que_e = torch.zeros(self.h_dim).float().cuda() if use_cuda else torch.zeros(self.h_dim).float()
    #     subg_index = torch.masked_select(
    #             torch.arange(0, sub_graph.number_of_nodes(), dtype=torch.long).cuda(),
    #             (sub_graph.in_degrees(range(sub_graph.number_of_nodes())) > 0))
    #     if subg=='None':
    #         que_subg.append(que_e.unsqueeze(0))
    #     else:
    #         sub_graph.ndata['h'] = ent_emb
    #         subg_emb= self.rgat_layer.forward(sub_graph, ent_emb, self.emb_rel)
    #         subg_emb= self.rgat_layer.forward(sub_graph, subg_emb, self.emb_rel)
        
    #     return F.normalize(subg_emb),subg_index
