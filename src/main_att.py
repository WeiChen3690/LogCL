...
from src.rrgcn_att import RecurrentRGCN
...
def update_dict(subg_arr, s_to_sro, sr_to_sro,sro_to_fre, num_rels):
    ...
    for j, (src, rel, dst) in enumerate(subg_triples):
        ...
        # if (src, rel, dst) not in  sro_to_fre.keys():
        #     sro_to_fre[(src, rel, dst)]=1
        # else:
        #     sro_to_fre[(src, rel, dst)] = sro_to_fre[(src, rel, dst)]+1
...
def test(model, history_list, test_list, num_rels, num_nodes, use_cuda, all_ans_list, all_ans_r_list, model_name, static_graph, mode):
    ...
    #sr_to_sro = np.load('/mnt/nfs-storage-node-11/data/{}/his_dict/train_s_r.npy'.format(args.dataset), allow_pickle=True).item()
    ...
    for time_idx, test_snap in enumerate(tqdm(test_list)):
        ...
        if args.pre_type == "short":
            sub_snap,sub_snap_inv=[],[]
        else:
            # sub_snap = subgraph_arr[len(history_list)-time_idx-1]  #取出采样子图
            # sub_snap_inv = subgraph_arr_inv[len(history_list)-time_idx-1]
    ...
    if mode == "test": # test模式写入，train模式忽略
        ...
        #filename ='/mnt/nfs-storage-node-11/result/'+ args.dataset + "_new.csv"
...
def run_experiment(args, n_hidden=None, n_layers=None, dropout=None, n_bases=None):
    ...
    #model_state_file = '/mnt/nfs-storage-node-11/nni_experiments/models/'+model_name+".pt"
    ...
    if args.test and os.path.exists(model_state_file):
        ...
        for epoch in range(args.n_epochs):
            ...
            for train_sample_num in tqdm(idx):
                ...
                if args.pre_type == "short":
                    ...
                    #subgraph_arr = np.load('/mnt/nfs-storage-node-11/data/{}/his_graph_for/train_s_r_{}.npy'.format(args.dataset, train_sample_num))
                    #subgraph_arr_inv = np.load('/mnt/nfs-storage-node-11/data/{}/his_graph_inv/train_o_r_{}.npy'.format(args.dataset, train_sample_num))
            ...
            if epoch and epoch % args.evaluate_every == 0:
                ...
                nni.report_intermediate_result(float(mrr_filter))
            # Print the accuracy and memory usage
        ...
        mrr_raw, mrr_filter = test(model, 
                            ...
                            static_graph, 
                            mode="test")
        nni.report_final_result(float(mrr_filter))
...
if __name__ == '__main__':
    ...
    if len(RECEIVED_PARAMS) == 0:
        print("\n\n没有接收到参数\n\n")
    # 使用新参数更新原有参数
    args.__dict__.update(RECEIVED_PARAMS)
